...
describe('PUT /booking/:bookingId', () => {
    ...
    describe('when token is valid', () => {
        it('should respond with statuts 404 when roomId sent on body doesnt exist', async () => {
            ...
            const response = await server.put(`/booking/${booking.id}`).set('Authorization', `Bearer ${token}`).send({ roomId: 999999 });
        ...
        it('should respond with status 200 and with an object body containing bookingId', async () => {
            ...
            const response = await server.put(`/booking/${booking.id}`).set('Authorization', `Bearer ${token}`).send({ roomId: secondRoom.id });
